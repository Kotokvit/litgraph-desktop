...
pub fn tag_sentence(tokens: &[&str], candidates: &[Vec<PosTag>]) -> Vec<TaggedToken> {
    ...
    tokens
        ...
        .map(|((&word, cands), rule_id)| {
            // src-tauri variant: no lemmatizer module available, use word lowercase as fallback
...
fn apply_case_government(
    ...
    for i in 0..tokens.len() {
        ...
        // Get the verb lemma (src-tauri variant: lowercase the word form as fallback)
        let verb_lemma = tokens[i].to_lowercase();
